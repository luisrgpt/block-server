...
import secfs.common.exception.PublicKeyNotStoredException;
...
final class FileSystemClient {
	...
	static final int BLOCK_LENGTH = 300;
	
	private BlockId _fileId = null;
	private IFileSystemServer _blockServer = null;

	private Map<Integer, BlockId> _blockTable = new HashMap<>();
	private Map<Integer, BlockId> _previousBlockTable = new HashMap<>(); //Check integrity in the client side
	
	private int _bytesRead = 0;
	private IAuth _cc_Auth= new CC_Auth();
	...
	private IFileSystemServer getBlockServer()
		...
		if (_blockServer == null) {
			...
			_blockServer = (IFileSystemServer)registry.lookup(IFileSystemServer.SERVICE_NAME);
		}
		return _blockServer;
	}

	private Map<Integer, BlockId> getBlockTable(BlockId fileId)
			throws NotBoundException, TamperedBlockException, RemoteException, InvalidKeyException, NoSuchAlgorithmException, SignatureException, InvalidRemoteArgumentException {
		try {
	...
	//only applicable to hash blocks
  	private FileBlock getAndVerifyHashBlock(BlockId blockId, BlockId prevBlockId)
  			throws NoSuchAlgorithmException, TamperedBlockException, RemoteException, BlockNotFoundException, NotBoundException, InvalidRemoteArgumentException{
	...
	//only applicable to hash blocks
  	private FileBlock getAndVerifyKeyBlock(BlockId fileId)
  			throws NoSuchAlgorithmException, TamperedBlockException, InvalidKeyException, SignatureException, RemoteException, BlockNotFoundException, NotBoundException, InvalidRemoteArgumentException{
...
//    	signature.update(data);
  	...
  	private void sendIndexBlocks(Map<Integer, BlockId> map)
  			throws RemoteException, NotBoundException, NoSuchAlgorithmException, InvalidKeyException, SignatureException, InvalidKeySpecException, TamperedBlockException, InvalidRemoteArgumentException {
...
//					signature.update(keyBlock.getBytes());
	...
	void initializeFileSystemClient()
			throws RemoteException, CertificateException, KeyStoreException, InvalidRemoteArgumentException, NoSuchAlgorithmException, NotBoundException, PublicKeyNotStoredException {
		//Submit certificate
	...
	void writeIntoFileSystemServer(int pos, int size, byte[] contents)
			throws BlockNotFoundException, TamperedBlockException, RemoteException, NoSuchAlgorithmException, NotBoundException, InvalidKeyException, SignatureException, InvalidKeySpecException, InvalidRemoteArgumentException {
		IFileSystemServer blockServer = getBlockServer();
	...
	int readFromFileSystemServer(byte[] id, int pos, int size, byte[] contents)
			throws TamperedBlockException, RemoteException, NotBoundException, InvalidKeyException, NoSuchAlgorithmException, SignatureException, BlockNotFoundException, InvalidRemoteArgumentException {
		_bytesRead = 0;
		IFileSystemServer blockServer = getBlockServer();
	...
	List<PublicKey> getPublicKeysFromFileSystemServer()
			throws RemoteException, NotBoundException, KeyStoreException, NoSuchAlgorithmException, InvalidKeySpecException {
		IFileSystemServer blockServer = getBlockServer();
